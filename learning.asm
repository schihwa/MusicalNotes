...
staveMessage: .asciiz "this is a test"
...
addi $s0, $s0, 262144      # skip the first 8 pixels (32 bytes)
...
loop:
    ...
    addi $t1, $t1, 1     # increment the loop counter
    addi $s0, $s0, 4     # increment the memory address pointer by 1 byte (the size of a pixel)
    bne $t1, 512, loop # loop until the counter reaches 25600 (the number of pixels in the bitmap) 
